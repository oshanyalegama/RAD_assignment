...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        hyp = {
            ...
            "obj": 2.1,  # obj loss gain (scale with pixels)
...
class ComputeNewLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        lbox = torch.zeros(1, device=self.device)  # box loss
        lobj = torch.zeros(1, device=self.device)  # object loss
        lcent = torch.zeros(1, device=self.device)  # center alignment loss
        laspect = torch.zeros(1, device=self.device)  # aspect ratio loss
        tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n := b.shape[0]:
                ...
                # Center alignment (new term)
                pcenter = pxy  # Predicted center
                tcenter = tbox[i][:, :2]  # Target center
                center_loss = torch.mean((pcenter - tcenter) ** 2)  # L2 distance between predicted and target centers
                lcent += center_loss
        ...
        lobj *= self.hyp["obj"]
        lcls *= self.hyp["cls"]
        lcent *= self.hyp["cent"]  # Center alignment scaling factor
        laspect *= self.hyp["aspect"]  # Aspect ratio scaling factor
        bs = tobj.shape[0]  # batch size
    
        return (lbox + lobj + lcls + lcent + laspect) * bs, torch.cat((lbox, lobj, lcls, lcent, laspect)).detach()
